...
通常情况下`application`需要缓存生成的数据，之后一次发送。不过对于`WSGI application`无法这样做，`WSGI application`仅返回一个`iterable`。

对于大体积数据，如果需要将其分解为若干个部分发送，`WSGI Server & Middleware`必须直接向`client`传递，不能延迟。

> applicaiton的返回值并不是直接返回给了client

TODO: 感觉也没讲`server`如何缓存数据啊。
...
https://www.python.org/dev/peps/pep-3333/#middleware-handling-of-block-boundaries